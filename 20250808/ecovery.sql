...
DROP TABLE IF EXISTS env_img;
DROP TABLE IF EXISTS env;
DROP TABLE IF EXISTS notice;
DROP TABLE IF EXISTS member;
DROP TABLE IF EXISTS disposal_history;
DROP TABLE IF EXISTS disposal_history_img;
DROP TABLE IF EXISTS disposal_feedback;

SELECT * FROM member;
SHOW INDEX FROM member WHERE Column_name = 'nickname';

DELETE FROM member WHERE member_id = 10;

-- test@test.com 관리자로 권한 변경
UPDATE member
SET role = 'ADMIN'
WHERE member_id = 1;
